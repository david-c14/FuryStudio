...
TEST_CASE("Given a faulty imm When a bmp is created Then an exception is raised") {
	...
	bmp_p bmp = Test_Bmp_createFromImmAndPam(pixelFile.data(), uint32_t(pixelFile.size()), paletteFile.data(), uint32_t(paletteFile.size()), 1);
	REQUIRE(bmp == NULL);
	REQUIRE(Test_Exception_code() == 1);
...
TEST_CASE("Given a sound bmp When used to create an imm Then the correct buffers are returned") {
	...
	try {
		...
		uint8_t result2 = Test_Imm_pamBuffer(bmp, actualPaletteFile.data(), uint32_t(actualPaletteFile.size()), 1);
